...
import { ArrowUpRight, Users, GraduationCap, CalendarDays, Wallet, CheckCircle2, Clock3, XCircle, Inbox } from "lucide-react";
...
type Member = { id: string; first_name: string | null; last_name: string | null; city: string | null; country: string | null; category: string; created_at: string };
...
function DashboardPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      const [{ count: total }, { count: pending }, { count: week }, { data: txData }, { data: evData }, { count: upc }, { data: nm }] = await Promise.all([
        supabase.from("profiles").select("*", { count: "exact", head: true }),
        supabase.from("profiles").select("*", { count: "exact", head: true }).eq("status", "En attente"),
        supabase.from("profiles").select("*", { count: "exact", head: true }).gte("created_at", weekAgo),
        supabase.from("transactions").select("id,user_id,reason,amount,currency,method,status,occurred_at").order("occurred_at", { ascending: false }).limit(5),
        supabase.from("events").select("id,title,event_date,location").gte("event_date", now).order("event_date", { ascending: true }).limit(4),
        supabase.from("events").select("*", { count: "exact", head: true }).gte("event_date", now),
        supabase.from("profiles").select("id,first_name,last_name,city,country,category,created_at").order("created_at", { ascending: false }).limit(4),
      ]);
      ...
      const { data: monthTx } = await supabase.from("transactions").select("amount,status").gte("occurred_at", monthStart).eq("status", "Réussi");
      setMonthSum((monthTx ?? []).reduce((s: number, t: { amount: number }) => s + Number(t.amount), 0));
      ...
      if (txData && txData.length) {
        ...
        const nameById = new Map((profs ?? []).map((p) => [p.id, [p.first_name, p.last_name].filter(Boolean).join(" ") || "Membre"]));
  ...
  return (
    <div className="max-w-[1400px] mx-auto space-y-6">
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard icon={Users} label="Membres" value={String(memberCount)} tone="oklch(0.55 0.22 255)" />
        <StatCard icon={GraduationCap} label="Inscriptions (7j)" value={String(weekJoins)} tone="oklch(0.7 0.16 165)" />
        <StatCard icon={CalendarDays} label="Événements à venir" value={String(upcomingCount)} tone="oklch(0.75 0.15 75)" />
        <StatCard icon={Wallet} label="Encaissé (mois)" value={fmtMoney(monthSum)} tone="oklch(0.6 0.22 295)" />
      </div>
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 bg-card rounded-2xl border border-border p-6 shadow-[var(--shadow-card)]">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link to="/finances" className="text-sm text-primary hover:underline inline-flex items-center gap-1">Voir tout <ArrowUpRight className="h-3.5 w-3.5" /></Link>
          </div>
          {txs.length === 0 ? (
              ...
              {txs.map((t) => (
                <div key={t.id} className="py-3 flex items-center gap-4">
                  ...
                  <div className="flex-1">
                    ...
                    <div className="text-xs text-muted-foreground">{t.reason}{t.method ? ` • ${t.method}` : ""}</div>
                  </div>
                  <div className="text-right">
                    <div className="font-semibold tabular-nums">{new Intl.NumberFormat("fr-FR").format(Number(t.amount))} {t.currency}</div>
                    <div className="text-xs text-muted-foreground">{new Date(t.occurred_at).toLocaleDateString("fr-FR")}</div>
                  </div>
                </div>
              ))}
            </div>
        ...
        {newMembers.length === 0 ? (
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-3">
            {newMembers.map((m) => {
              ...
              return (
                <Link key={m.id} to="/membres/$memberId" params={{ memberId: m.id }} className="p-4 rounded-xl border border-border hover:border-primary hover:bg-primary-soft/50 transition">
                  <div className="flex items-center gap-3">
                    <Avatar name={name} />
                    <div className="min-w-0">
                      <div className="font-semibold text-sm truncate">{name}</div>
                      <div className="text-xs text-muted-foreground">{[m.city, m.country].filter(Boolean).join(", ") || "—"}</div>
                    </div>
                  </div>
                  <div className="mt-3 text-xs inline-flex px-2 py-0.5 rounded-full bg-primary-soft text-primary font-medium">{m.category}</div>
                </Link>
              );
            })}
          </div>
